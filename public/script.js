const translations = {
  hr: {
    'nav.project':'O PROJEKTU','nav.apartments':'STANOVI','nav.location':'LOKACIJA','nav.contact':'KONTAKT','nav.contactUs':'KONTAKTIRAJTE NAS',
    ...
    'closing.text':'Mjesto gdje<br>povijest i more<br>žive zajedno.','footer.rights':'Sva prava pridržana.','modal.floorplan':'TLOCRT','modal.isometric':'3D IZOMETRIJSKA SHEMA','modal.apartment':'STAN','modal.download':'DOWNLOAD PDF KATALOG','modal.variant':'VARIJANTA',
    'modal.sold':'PRODANO'
  },
  en: {
    'nav.project':'THE PROJECT','nav.apartments':'APARTMENTS','nav.location':'LOCATION','nav.contact':'CONTACT','nav.contactUs':'CONTACT US',
    ...
    'closing.text':'A place where<br>history and the sea<br>live together.','footer.rights':'All rights reserved.','modal.floorplan':'FLOOR PLAN','modal.isometric':'3D ISOMETRIC SCHEMA','modal.apartment':'APARTMENT','modal.download':'DOWNLOAD PDF CATALOGUE','modal.variant':'VARIANT',
    'modal.sold':'SOLD'
  }
};
...
function applyTranslations(){
  ...
  document.title=currentLang==='hr'?'Vrota Poja | Stanovi u Starom Gradu na Hvaru':'Vrota Poja | Apartments in Stari Grad, Hvar';
  const meta=document.querySelector('meta[name="description"]');
  meta.content=currentLang==='hr'?'Vrota Poja — moderno stanovanje u srcu povijesti. Novi stanovi u Starom Gradu na Hvaru, s terasama, vrtovima i parkingom.':'Vrota Poja — modern living in the heart of history. New apartments in Stari Grad, Hvar, with terraces, gardens and parking.';
...
function openCard(card){
  ...
  modalIso.src=`isometric/${card.dataset.code}.jpg`;
  modalIso.alt=`${t('modal.isometric')} ${card.dataset.name}`;
